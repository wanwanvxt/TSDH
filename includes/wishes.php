...
{
  $db = new database();
  if ($db->execute("INSERT INTO wishes(user_id, um_id) SELECT ?, u_m.id FROM u_m LEFT JOIN wishes ON u_m.id = wishes.um_id AND wishes.user_id = ? WHERE u_m.univer_id = ? AND u_m.major_id = ? AND wishes.user_id IS NULL;", "ssss", $user_id, $user_id, $univer_id, $major_id)) {
